...
if __name__ == "__main__":
    ...
    filtered_image_algorithm_color = median_filter_algorithm_color(
        input_image_color["image_data"], kernel_size=3
    )

    filtered_image_opencv_color = median_filter_opencv(
        input_image_color["image_data"], kernel_size=3
    )
    ...
    print(
        f"Ảnh màu sau khi áp dụng median Filter (thuật toán) đã được lưu tại: {output_path_algorithm_color}"
    )
    print(
        f"Ảnh màu sau khi áp dụng median Filter (OpenCV) đã được lưu tại: {output_path_opencv_color}"
    )
